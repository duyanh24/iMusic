...
class FirebaseAccount {
    ...
    private func authencationAccount(email: String, password: String) {
        Auth.auth().signIn(withEmail: email, password: password) { [weak self] result, error in
            ...
            if self.checkValidateAccount(email: email, password: password) {
                if let error = error {
                ...
                self.loginResult.onNext(.failure(APIError(status_code: nil, status_message: ErrorMessage.validateError)))
            }
        }
    }
    
    private func checkValidateAccount(email: String, password: String) -> Bool {
